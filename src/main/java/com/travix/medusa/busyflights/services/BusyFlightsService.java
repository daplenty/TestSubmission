...
public class BusyFlightsService implements HostGateway{
    ...
    public List<BusyFlightsResponse> searchForFlight(BusyFlightsRequest busyFlightsRequest) {
        ...
        List<BusyFlightsResponse> busyFlightsResponss = new ArrayList<>();
        for(Supplier services :supplierServices ){
        
            List<BusyFlightsResponse> reslt = services.searchForFlight(busyFlightsRequest);
            busyFlightsResponss.addAll(reslt);
        }
        
        supplierServices.stream().map((provider) -> {
            return provider.searchForFlight(busyFlightsRequest);
        }).forEach((results) -> {
